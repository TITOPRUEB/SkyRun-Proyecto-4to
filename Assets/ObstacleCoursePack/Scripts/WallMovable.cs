...
public class WallMovable : MonoBehaviour
{
	...
	public float height = 15; //Height of the platform
	public float posYDown = 15; //Start position of the Y coord
    ...
    void Update()
    {
		if (isDown)
		{
			if (transform.position.y < posYDown + height)
			{
				...
				StartCoroutine(WaitToChange(0.15f));
		}
		else
		{
			if (!canChange)
